...
class TestUSGSClient:
    ...
    async def test_get_sites_by_address_denver(self, client):
        ...
        print(
            f"Geocoded '{location_info['address']}' to: {location_info['resolved_address']}"
        )
        print(
            f"Coordinates: {location_info['latitude']:.4f}, {location_info['longitude']:.4f}"
        )
    ...
    async def test_get_sites_by_address_specific_location(self, client):
        ...
        print(
            f"Boulder geocoded to: {location_info['latitude']:.4f}, {location_info['longitude']:.4f}"
        )
    ...
    async def test_invalid_address_handling(self, client):
        ...
        async with client as usgs:
            with pytest.raises(USGSException):  # Should raise USGSException
